...
// import {ReactComponent as PersonIcon} from '../../../../components/Dashboard/StakticsData/Icons/Person1.svg';
  ...
  return (  
    ...
    <Grid container boxShadow={4} borderRadius={4} p={2} >
        ...
        {/* <Typography px={1} variant='body1' fontWeight={500} >
          ...
          {
             name.length > 48 ? <Tooltip sx={{ fontSize: '1.5rem !important' }} title={ name}>
              <Typography sx={{ fontSize: '13px !important',fontWeight:'600', color: 'black',cursor:'pointer',textAlign:'left' }}   >{ name.slice(0,48)}</Typography>
        ...
        <Typography  px={1} variant='button' fontSize={'12px'} fontWeight={900} color={'black'} >{
            responsibleName
            }</Typography>
        </Box>
        </Grid>
        ...
        bgcolor={ objectivesStateName === "فعال"?'#D5f7D4':'#bfd3f5'}   width={'85px'} height={'25px'} borderRadius={3} >
        <Typography  color={objectivesStateName === "فعال"? 'green':'#3a82fc'} fontSize={'0.75rem'}  >  {
        ...
        <Box display={'flex'} alignItems={'center'} textAlign={'center'}  justifyContent={'center'} bgcolor={'#FBF0EA'}  
        width={'90px'} height={'25px'} borderRadius={3} >

        <Typography color={'#F95700'} fontSize={'0.75rem'}>
        ...
        <Typography fontSize={'0.75rem'} color={'#0d0d0c'}>
         نمایش جزئیات
         </Typography>
         </Button>
